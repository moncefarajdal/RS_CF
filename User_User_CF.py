...
K = 50 # number of neighbors we'd like to consider
# limit = 5 # number of common movies users must have in common in order to consider
limit = 10 # number of common movies users must have in common in order to consider
...
for user_idx in user_indices_for_recommendation:
    ...
    for movie_idx in range(M):
        ...
        actual_rating = usermovie2rating.get((user_idx, movie_idx), None)
...
recommendation_data = []
for user_idx, recommended_movies in recommendations:
    for movie_idx, prediction, actual_rating in recommended_movies:
        recommendation_data.append([user_idx, movie_idx, prediction, actual_rating])

recommendation_df = pd.DataFrame(recommendation_data, columns=['User', 'Movie', 'Predicted Rating', 'Actual Rating'])
recommendation_df.to_csv('User_user_CF_recommendations.csv', index=False)

print("Recommendations saved to 'User_user_CF_recommendations.csv'")
