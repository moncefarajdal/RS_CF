...
# we can use the trained parameters to predict the ratings for all the items
# for each of the 10 users and then recommend the top-rated items.
# Here is the code to perform recommendation for 10 users
# and show the actual rating next to predicted rating saved into a csv file

# Choose 10 users for recommendation
...
for user_idx in user_indices_for_recommendation:
    ...
    for item_idx in range(M):
        # Predict the rating for this user-item pair
        prediction = predict(item_idx, user_idx)
        actual_rating = usermovie2rating.get((user_idx, item_idx), None)
...
recommendation_data = []
for user_idx, recommended_items in recommendations:
    for item_idx, prediction, actual_rating in recommended_items:
        recommendation_data.append([user_idx, item_idx, prediction, actual_rating])

recommendation_df = pd.DataFrame(recommendation_data, columns=['User', 'Item', 'Predicted Rating', 'Actual Rating'])
recommendation_df.to_csv('Item_item_CF_recommendations.csv', index=False)

print("Item recommendations saved to 'Item_item_CF_recommendations.csv'")
